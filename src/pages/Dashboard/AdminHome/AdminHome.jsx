...
const AdminHome = () => {
    ...
    const { data: chartData = [] } = useQuery({
        ...
        queryFn: async () => {
            const res = await axiosSecure('/order-stats');
            // Modify the data received to include the 'count' property
            const modifiedData = res.data.map((entry) => ({
                ...entry,
                count: entry.total, // Use the appropriate property for the count
            }));
            return modifiedData;
        },
    });
    ...
    return (
        <div className="w-full m-4 ml-10">
